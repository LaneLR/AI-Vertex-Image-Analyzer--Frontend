...
export default function CalculatorClient(
    { history }: { history: ScanHistoryItem[] }
) {
  ...
  const netProfit = numericSell - numericBuy - numericShipping - totalFees - numericOther;
  ...
  return (
    <>
        ...
        {/* <Calculator size={24} className="icon-gold" /> */}
        <div />
      </header>
      ...
      <div className="profit-calc__content">
        ...
        <section className="calc-card">
          <div className="calc-card__group">
            ...
            <select className="calc-input" onChange={handleSelectItem} value={selectedItem?.id || ""}>
          ...
          {selectedItem && (
            <div className="selected-item-preview">
              {selectedItem.imageUrl && <img src={selectedItem.imageUrl} alt="Item" />}
              <div className="selected-item-preview__info">
                <h4>{selectedItem.title}</h4>
                <p>Est. Range: ${selectedItem.lowResellValue} - ${selectedItem.highResellValue}</p>
              </div>
              <button className="btn-clear" onClick={() => setSelectedItem(null)}>
                <Trash2 size={26} />
              </button>
            </div>
          )}
        ...
        <div className="calc-grid">
          <section className="calc-card">
            ...
            <div className="calc-card__group">
              ...
              <input type="number" value={buyCost} onChange={(e) => setBuyCost(e.target.valueAsNumber || "")} placeholder="0.00" className="calc-input" />
            </div>

            <div className="calc-card__group">
              <label>Potential Sell Price ($)</label>
              <input type="number" value={sellPrice} onChange={(e) => setSellPrice(e.target.valueAsNumber || "")} placeholder="0.00" className="calc-input" />
            </div>

            <div className="calc-card__group">
              <label>Shipping & Materials ($)</label>
              <input type="number" value={shippingCost} onChange={(e) => setShippingCost(e.target.valueAsNumber || "")} placeholder="0.00" className="calc-input" />
            </div>
          </section>
          ...
          <section className="calc-card">
            ...
            <div className="calc-card__group">
              ...
              <select className="calc-input" onChange={(e) => setPlatformFeePercent(parseFloat(e.target.value))}>
                {PLATFORMS.map(p => <option key={p.name} value={p.fee}>{p.name} ({p.fee}%)</option>)}
              </select>
            </div>

            <div className="calc-card__group">
              <label>Fee Percentage (%)</label>
              <input type="number" placeholder="0" value={platformFeePercent} onChange={(e) => setPlatformFeePercent(e.target.valueAsNumber || 0)} className="calc-input" />
            </div>

            <div className="calc-card__group">
              <label>Other Costs (Gas, Promoted Listings, etc.)</label>
              <input type="number" value={otherCosts} onChange={(e) => setOtherCosts(e.target.valueAsNumber || "")} placeholder="0.00" className="calc-input" />
            </div>
          </section>
        </div>

        {/* SECTION: Result Display */}
        <section className={`result-display ${netProfit > 0 ? 'result-display--pos' : netProfit < 0 ? 'result-display--neg' : "result-display--neutral" }`}>
